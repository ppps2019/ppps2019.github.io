...
[Online Exhibitor Registration](http://www.cvent.com/d/9gqc5t/4W)
...
| -------------:| ------------- | ------- |{% for exhibitor_hash in site.data.exhibitors %}{% assign exhibitor = exhibitor_hash[1] %}
| {{ exhibitor.booth }} | {{ exhibitor.name }} | [{{exhibitor.url}}]({{exhibitor.url}}) |{% endfor %}


# PPPS 2019 Exhibitors

{% for exhibitor_hash in site.data.exhibitors %}
{% assign exhibitor = exhibitor_hash[1] %}
{% if exhibitor.logo %}
Booth {{ exhibitor.booth }}: {{ exhibitor.name }}<br><a href="{{exhibitor.url}}">
![{{ exhibitor.name }} logo]({{ site.url }}/images/exhibitors/{{ exhibitor.logo }}){: class="exlogo" alt="{{exhibitor.name}}"}</a><br>
{% endif %} 
{% endfor %}
